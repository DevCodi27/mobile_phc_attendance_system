...
public class PolygonConverter {
    ...
    private static final GeometryFactory geometryFactory = new GeometryFactory(new PrecisionModel(), 4326); // SRID 4326 for GPS
    ...
    public static Polygon convertToPolygon(List<LocationDTO> coords) {
        ...
        for (int i = 0; i < coords.size(); i++) {
            coordinates[i] = new Coordinate(coords.get(i).getLongitude(), coords.get(i).getLatitude()); // (x, y) => (lng, lat)
        }
